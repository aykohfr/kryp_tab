...
local function CreateFonts()
    surface.CreateFont("FreeTablist.Server", {
        font = "Roboto",
        size = Scale(28),
        weight = 800,
        antialias = true,
    ...
    surface.CreateFont("FreeTablist.Subtitle", {
        font = "Roboto",
        size = Scale(15),
        weight = 500,
        antialias = true,
    ...
    surface.CreateFont("FreeTablist.Header", {
        font = "Roboto",
        size = Scale(13),
        weight = 700,
        antialias = true,
    ...
    surface.CreateFont("FreeTablist.Player", {
        font = "Roboto",
        size = Scale(16),
        weight = 650,
        antialias = true,
    ...
    surface.CreateFont("FreeTablist.Small", {
        font = "Roboto",
        size = Scale(13),
        weight = 500,
        antialias = true,
    ...
    surface.CreateFont("FreeTablist.Button", {
        font = "Roboto",
        size = Scale(11),
        weight = 750,
        antialias = true,
...
CreateFonts()
hook.Add("OnScreenSizeChanged", "FreeTablist.RecreateFonts", CreateFonts)
...
local function MakeButton(parent, text, width, onClick)
    ...
    button.Paint = function(self, w, h)
        ...
        draw.SimpleText(self.Label or text, "FreeTablist.Button", w / 2, h / 2, hovered and C("Text", color_white) or C("Muted", Color(145, 145, 145)), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    end
...
local function CreatePlayerRow(parent, ply)
    ...
    name:SetFont("FreeTablist.Player")
    ...
    role:SetFont("FreeTablist.Small")
    ...
    local function StatLabel(x, title, valueFunc)
        ...
        label:SetFont("FreeTablist.Small")
...
function FT:BuildPlayerList()
    ...
    if count == 0 then
        ...
        empty:SetFont("FreeTablist.Small")
...
function FT:Open()
    ...
    serverName:SetFont("FreeTablist.Server")
    ...
    playerCount:SetFont("FreeTablist.Subtitle")
    ...
    search:SetFont("FreeTablist.Small")
    ...
    columnHeader.Paint = function(_, w, h)
        draw.SimpleText("JOUEUR", "FreeTablist.Header", Scale(72), h / 2, C("Muted", Color(143, 147, 158)), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        draw.SimpleText("STATISTIQUES", "FreeTablist.Header", Scale(330), h / 2, C("Muted", Color(143, 147, 158)), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        draw.SimpleText("ACTIONS", "FreeTablist.Header", w - Scale(262), h / 2, C("Muted", Color(143, 147, 158)), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    end
    ...
    footer:SetFont("FreeTablist.Small")
    ...
    timer.Remove("FreeTablist.Refresh")
    timer.Create("FreeTablist.Refresh", CONFIG.RefreshInterval or 1, 0, function()
...
hook.Add("ScoreboardShow", "FreeTablist.Open", function()
    FT:Open()
    return true
end)

hook.Add("ScoreboardHide", "FreeTablist.Close", function()
    FT:Close()
end)

hook.Add("ShutDown", "FreeTablist.Cleanup", function()
    timer.Remove("FreeTablist.Refresh")
end)
